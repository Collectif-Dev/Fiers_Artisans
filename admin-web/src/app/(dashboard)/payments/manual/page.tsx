...
import { ChevronLeft, ChevronRight, WalletCards, AlertTriangle } from 'lucide-react';
...
function parseImageRef(ref?: string): { bucket: string; objectKey: string } | null {
  if (!ref) return null;
...
export default function PaymentManualPage() {
  ...
  const statusBadge = (record: PaymentManualRecord) => {
    ...
    if (record.status === 'COMPLETED') {
      return <Badge className="bg-green-100 text-green-900">{t('status_completed')}</Badge>;
    }
    if (record.status === 'REJECTED') {
      return <Badge variant="destructive">{t('status_rejected')}</Badge>;
    }
    if (record.status === 'PENDING_ADMIN') {
      return <Badge className="bg-yellow-100 text-yellow-900">{t('status_pending_admin')}</Badge>;
    }
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {t('title')}
            {!loading && <Badge variant="secondary">{items.length}/{total}</Badge>}
          </CardTitle>
        ...
        <CardContent>
          {loading ? (
            ...
            <p className="text-sm text-muted-foreground py-10 text-center">{t('no_data')}</p>
          ) : (
            <>
                  ...
                  <TableBody>
                    {items.map((item) => (
                      <TableRow key={item.id}>
                        ...
                        <TableCell>
                          {item.subscription?.artisan_profile?.user?.phone_number || '—'}
                        </TableCell>
                        <TableCell>
      ...
      <Dialog open={detailsOpen} onOpenChange={(open) => (!open ? closeDetails() : setDetailsOpen(true))}>
        <DialogContent className="max-w-3xl max-h-[88vh] overflow-y-auto">
          ...
          <DialogFooter className="flex flex-wrap gap-2">
            ...
            <Button onClick={() => void applyAction('validate')} disabled={actionLoading || !selected || selected.status !== 'PENDING_ADMIN'}>
              {t('validate')}
            </Button>
            <Button
              variant="destructive"
              onClick={() => void applyAction('reject')}
              disabled={actionLoading || !selected || selected.status !== 'PENDING_ADMIN'}
            >
              {t('reject')}
            </Button>
            <Button
